...
async function request(path, init = {}) { let lastError; for (let attempt = 0; attempt < 8; attempt += 1) { try { const response = await fetch(`${url}${path}`, { ...init, headers:{ ...authHeaders, ...(init.headers ?? {}) }, signal:AbortSignal.timeout(30000) }); if (!response.ok) throw new Error(`${response.status}: ${await response.text()}`); return response; } catch (error) { lastError = error; if (attempt < 7) { const seconds = Math.min(30, 2 ** attempt); console.log(`网络波动，${seconds} 秒后自动重试…`); await new Promise((resolve) => setTimeout(resolve, 1000 * seconds)); } } } throw lastError; }
...
const batchSize = Math.max(1, Math.min(5, Number(process.env.IMPORT_CONCURRENCY ?? 2)) || 2);
for (let offset = 0; offset < records.length; offset += batchSize) { const created = await Promise.all(records.slice(offset, offset + batchSize).map(provision)); teacherAccounts.push(...created.filter(Boolean)); console.log(`已处理 ${Math.min(offset + batchSize, records.length)} / ${records.length} 个账户`); }
