...
Usage: SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... node scripts/import-roster.mjs C:\path\roster.txt C:\path\thesis-groups.tsv 初始密码
...
if (!filePath || !groupPath || !initialPassword || !url || !serviceKey) throw new Error("缺少名单文件、A23/Z25分组文件、初始密码或 Supabase 配置。");
...
for (const record of [...students, ...teachers]) { const email = `${record.identity_no.toLowerCase()}@selection.local`; const account = await post("/auth/v1/admin/users", { email, password:initialPassword, email_confirm:true, user_metadata:{identity_no:record.identity_no} }); await post("/rest/v1/profiles", { id:account.id, ...record, active:true }); if (record.role === "teacher") await post("/rest/v1/teachers", { profile_id:account.id, active:true }); }
