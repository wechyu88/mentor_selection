...
async function provision(record) { const email = `${record.identity_no.toLowerCase()}@selection.local`; const account = await post("/auth/v1/admin/users", { email, password:record.role === "admin" ? adminPassword : initialPassword, email_confirm:true, user_metadata:{identity_no:record.identity_no} }); await post("/rest/v1/profiles", { id:account.id, ...record, active:true, must_change_password:record.role !== "admin" }); if (record.role === "teacher") { await post("/rest/v1/teachers", { profile_id:account.id, active:true }); return { id:account.id, name:record.full_name }; } }
